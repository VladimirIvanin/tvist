...
::: info Нативный `loading="lazy"` на `<img>`
Если вы **не** используете `data-src` / модуль LazyLoad, а ставите обычные `<img src="..." loading="lazy">`, см. опцию **`nativeLazyAdjacent`** в [опциях API](/api/options#native-lazy-adjacent) — она помогает при эффектах вроде **cube**, когда грань видна до срабатывания браузерного lazy.
:::
