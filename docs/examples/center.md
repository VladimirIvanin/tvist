...
`center: { focus: true }` стремится поставить **активный** слайд по центру viewport, но **не** создаёт пустоты у краёв: на первых и последних слайдах позиция обрезается до `min`/`max` scroll (аналог Splide `trimSpace: true`).

Отличие от `center: true` / `active: true`:

| Режим | Первый / последний слайд | Середина |
|--------|--------------------------|----------|
...
> **Важно:** В режиме `center: true` (и `active: true`) правила `peekTrim` для выравнивания слайдов по краям **не применяются** — активный слайд всегда по центру. В режиме `center: { focus: true }` trim у краёв включён (как у Splide с `trimSpace`).
...
interface TvistOptions {
   ...
   * - объект: отдельные флаги active/focus/justify
   * @default false
   */
  center?: boolean | {
    active?: boolean
