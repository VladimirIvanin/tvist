...
describe('Marquee + Drag + Loop: Локализация проблемы с дырами', () => {
  ...
  describe('ТЕСТ 2: Минимальный drag вправо', () => {
    it('location должен корректно изменяться при малом drag вправо', async () => {
      slider = new Tvist(root, {
        marquee: {
          speed: 100,
          direction: 'left'
      ...
      console.log('\n=== ТЕСТ 2: Малый drag вправо (30px) ===')
      const initialState = loopModule.getTransformState()
      console.log('До драга:', {
        location: initialState.location,
        slidesOrder: initialState.slidesOrder.join(', ')
      })

      const locationBefore = initialState.location

      // Драг вправо на 30px
      await simulateDrag(root, 400, 30, 3)

      const finalState = loopModule.getTransformState()
      console.log('После драга:', {
        location: finalState.location,
        slidesOrder: finalState.slidesOrder.join(', '),
        transform: finalState.transform
      })

      // ПРОВЕРКА: location должен увеличиться (drag вправо = положительное смещение)
      // В режиме marquee+loop loopFix не вызывается при драге,
      // поэтому location может быть положительным при старте с 0.
      // Когда marquee возобновляется, он сам перестраивает слайды.
      expect(finalState.location).toBeGreaterThan(locationBefore)
    })
  ...
  describe('ТЕСТ 5: Конфликт между marquee и loop логикой', () => {
    ...
    it('engine.location должен совпадать с реальным transform', async () => {
      slider = new Tvist(root, {
        marquee: {
          speed: 100,
          direction: 'left'
      ...
      console.log('\n=== ПРОВЕРКА СИНХРОНИЗАЦИИ LOCATION И TRANSFORM ===')

      await simulateDrag(root, 400, 80, 8)

      const location = slider.engine.location.get()
      const transform = getTranslateX(container)
      ...
      if (Math.abs(transform - expectedTransform) > tolerance) {
        ...
        console.log('   Разница:', Math.abs(transform - expectedTransform))
      }

      expect(Math.abs(transform - expectedTransform)).toBeLessThan(tolerance)
    })
  })

  describe('ТЕСТ 6: Критические сценарии', () => {
    it('множественные drag вправо-влево не должны накапливать ошибки', async () => {
      slider = new Tvist(root, {
        marquee: {
          speed: 100,
          direction: 'left'
      ...
      console.log('\n=== ТЕСТ 6: Множественные drag ===')

      const locations: number[] = []

      // Цикл drag вправо-влево
      for (let i = 0; i < 3; i++) {
        console.log(`\nИтерация ${i + 1}:`)
        ...
        locations.push(locationAfterRight)
        console.log('  После драга вправо:', locationAfterRight)

        // Драг влево
        await simulateDrag(root, 400, -50, 5)
        const locationAfterLeft = slider.engine.location.get()
        locations.push(locationAfterLeft)
        console.log('  После драга влево:', locationAfterLeft)
      }
    ...
    it('большой drag вправо должен корректно сдвигать позицию', async () => {
      ...
      console.log('\n=== ТЕСТ: БОЛЬШОЙ DRAG ВПРАВО ===')

      const locationBefore = slider.engine.location.get()

      // Очень большой drag вправо (300px)
      await simulateDrag(root, 400, 300, 30)

      const location = slider.engine.location.get()
      const transform = getTranslateX(container)

      console.log('Location до драга:', locationBefore)
