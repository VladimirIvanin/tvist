...
- `syncOnDrag` — синхронизация при перетаскивании (`true` по умолчанию). Если `false`, при перетаскивании (drag) одного слайдера другой не будет переключаться, синхронизация сработает только при клике или переключении стрелками.
